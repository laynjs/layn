import type { ItemId, Rect } from '@laynjs/core'
import {
...
export const createTransitionRunner = (
  ...
  const collect = (batch: TransitionCommit): TransitionPlan => {
    ...
    for (const index of batch.visible) {
      ...
      if (element === undefined || id === batch.skip) {
        continue
      }
      ...
      const move = moveOf(id, element, from, batch.next.rectAt(index))
      if (move !== undefined) {
        moves.push(move)
      }
    }
